...
- **Customise Seed Panel (Secret Mode)** – Hidden fork helper that loads default/store NPC + location data and exports fork-ready `data-npcs*.js` / `data-locations*.js` files.
...
- If you’re forking this repo, use the secret Customise panel to export fresh preload scripts for NPCs/Locations, then drop them into `js/data-npcs.js` and `js/data-locations.js`.
